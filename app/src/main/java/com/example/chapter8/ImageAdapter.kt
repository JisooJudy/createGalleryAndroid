...
class ImageAdapter(private val itemClickListener: ItemClickListener) : ListAdapter<ImageItems, RecyclerView.ViewHolder>(
    //ListAdapter에서 반드시 구현해야 하는 DiffUtil -> 아이템이 변경됐는지 확인(Different한지)
    object : DiffUtil.ItemCallback<ImageItems>(){
        override fun areItemsTheSame(oldItem: ImageItems, newItem: ImageItems): Boolean {//같은 객체를 참조하고 있는지 확인
...
sealed class ImageItems{//ImageItems가 sealed class로 Image데이터 클래스와 LoadMore 오브젝트를 가짐
    data class Image(
        val uri: Uri,
    ) : ImageItems()
